...
app.post('/to-html', function(req, res) {
	...
	fs.writeFile(path.join(dir + '/output.json'), json, function(err) {
		if (err) {
			console.log(err);
		} else {
			require('bootprint')
  				...
  				.build(path.join(dir + '/output.json'), dir)
  				.generate()
  				.done(function(result) {
  					console.log(result);
  					var ts = new Date().getTime();
  					new Inliner("http://localhost:8080/dist/html/index.html?" + ts, function (error, html) {
